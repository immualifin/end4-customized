...
Singleton {
    ...
    property var providers: {
        ...
        "wallhaven": {
            ...
            "description": Translation.tr("High quality wallpapers | Best for desktop, supports categories"),
            "mapFunc": (response) => {
            ...
            "fixedTags": [
                {"name": "anime", "count": "Anime category"},
                {"name": "general", "count": "General category"},
                {"name": "people", "count": "People category"},
                {"name": "landscape", "count": "Landscape"},
                {"name": "nature", "count": "Nature"},
                {"name": "space", "count": "Space"},
                {"name": "car", "count": "Cars"},
    ...
    function constructRequestUrl(tags, nsfw=true, limit=20, page=1) {
        ...
        if (currentProvider === "zerochan") {
            ...
            // Wallhaven API: q=query, categories=111 (general,anime,people), purity=100 (sfw)
            params.push("q=" + encodeURIComponent(tagString))
            params.push("categories=111") // All categories
            params.push("purity=" + (nsfw ? "110" : "100")) // 100=sfw, 110=sfw+sketchy
            ...
            if (apiKey && apiKey.length > 0) {
                ...
                if (nsfw) params[2] = "purity=111" // Include NSFW if key present
            }
        }
        else {
